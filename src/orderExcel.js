...
function buildColumnMap(headerRow) {
  return {
    ...
    createdAt: findColumn(headerRow, ['Дата создания', 'created']),
    dushanbeCity: findColumn(headerRow, ['Dushanbe City']),
    ...
    paidAlif: findColumn(headerRow, ['Alif']),
    paidCash: findColumn(headerRow, ['Наличные']),
...
function ordersToRows(orders) {
  ...
  orders.forEach((order) => {
    ...
    rows.push([
      ...
      order.comment || '',
      order.paid ? 'Да' : 'Нет',
      Number(payments['Наличные'] || 0),
      Number(payments.Alif || 0),
...
function rowsToOrders(rows) {
  ...
  return rows
    ...
    .map((row, rowIndex) => {
      ...
      const payments = {
        Наличные: parseNumber(readColumn(row, columns.paidCash)),
        Alif: parseNumber(readColumn(row, columns.paidAlif)),
        'Dushanbe City': parseNumber(readColumn(row, columns.dushanbeCity))
      };
...
export async function createOrdersWorkbook(orders) {
  const ExcelJS = await import('exceljs');
  const workbook = new ExcelJS.default.Workbook();
  const worksheet = workbook.addWorksheet('Заказы');
  const columnWidths = [10, 14, 14, 14, 10, 12, 42, 18, 28, 10, 12, 12, 16, 24, 10, 10];

  ordersToRows(orders).forEach((row) => worksheet.addRow(row));
