...
async function printText(text, printerName) {
  ...
  const command = printerName
    ? `Get-Content -LiteralPath '${filePath.replaceAll("'", "''")}' | Out-Printer -Name '${printerName.replaceAll("'", "''")}'`
    : `Get-Content -LiteralPath '${filePath.replaceAll("'", "''")}' | Out-Printer`;

  await new Promise((resolve, reject) => {
    runPowerShell(command).then(resolve).catch(reject);
  });
